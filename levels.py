...
def speedrun_level_easy(game_clocks, game_actors, actor_movement, actor_animation, game_questions, current_screen, sounds):
    if game_clocks.action == 0:
        for x in (250 + i * 150 for i in range(0, 3)):
            game_actors.new_actor(image_name="log", position=(x, randint(-800, -500)), actor_list=game_actors.logs, special_x=x, first=-800, last=-500)
        for i in range(0, 1):
            game_actors.new_actor(image_name='speed_powerup', position=(random.choice(actor_movement.DEFAULT_POSITIONS), randint(-1600, -1200)), actor_list=game_actors.powerups, first=-1600, last=-1200)
        game_clocks.countup()
    ...
    actor_movement.log_collision(actor_animation, game_actors, game_clocks, sounds, ts)
    actor_movement.speed_powerup_collision(game_actors, game_clocks, "stasis", 5, "strength", clock)
    ...
    #Actors movement
    actor_movement.reposition_actors(game_actors, game_actors.q_block, game_actors.powerups, game_actors.logs, quantity=2, special_effect=game_actors.powerup_collision, alt_quantity=3)
...
def points_level_easy(game_clocks, game_actors, actor_movement, actor_animation, game_questions, current_screen, sounds):
    if game_clocks.action == 0:
        for x in (250 + i * 150 for i in range(0, 3)):
            game_actors.new_actor(image_name="log", position=(x, randint(-800, -500)), actor_list=game_actors.logs, special_x=x, first=-800, last=-500)
        for i in range(0, 20): game_actors.new_actor(image_name="coin", position=(randint(175, 625), randint(-800, -300)), actor_list=game_actors.coins, first=-800, last=-300)
...
def speedrun_level_medium(game_clocks, game_actors, actor_movement, actor_animation, game_questions, current_screen, sounds):
    if game_clocks.action == 0:
        for x in (250 + i * 150 for i in range(0, 3)):
            game_actors.new_actor(image_name="log", position=(x, randint(-800, -500)), actor_list=game_actors.logs, special_x=x, first=-800, last=-500)
        for i in range(0, 1):
            game_actors.new_actor(image_name='speed_powerup', position=(random.choice(actor_movement.DEFAULT_POSITIONS), randint(-2400, -2000)), actor_list=game_actors.powerups, first=-2400, last=-2000)
    ...
    #Collisions with swimmer
    actor_movement.log_collision(actor_animation, game_actors, game_clocks, sounds, ts)
    actor_movement.speed_powerup_collision(game_actors, game_clocks, "stasis", 5, "monster", clock)
...
def points_level_medium(game_clocks, game_actors, comp_actors, actor_movement, actor_animation, game_questions, current_screen, sounds):
    if game_clocks.action == 0:
        for x in (250 + i * 150 for i  in range(0, 3)):
            game_actors.new_actor(image_name="log", position=(x, randint(-800, -500)), actor_list=game_actors.logs, special_x=x, first=-800, last=-500)
        for i in range(0, 20): game_actors.new_actor(image_name="coin", position=(randint(175, 625), randint(-800, -300)), actor_list=game_actors.coins, first=-800, last=-300)
        for i in range(0, 1): game_actors.new_actor(image_name='score_powerup', position=(random.choice(actor_movement.DEFAULT_POSITIONS), randint(-2000, -1600)), actor_list=game_actors.powerups, x_locked=True, first=-2000, last=-1600)
